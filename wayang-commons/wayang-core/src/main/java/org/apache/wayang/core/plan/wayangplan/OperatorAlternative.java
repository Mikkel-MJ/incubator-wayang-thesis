 ...
 * <p>
 * Alternatives and their interfaces (i.e., {@link OutputSlot}s and
 * {@link InputSlot}s) are matched via their
...
public class OperatorAlternative extends OperatorBase implements CompositeOperator, Serializable {
     ...
     * {@link OperatorAlternative}.
     */
    public class Alternative implements OperatorContainer, Serializable {

        /**
         * Maps the slots of the enclosing {@link OperatorAlternative} with this
         * instance.
         */
        private final SlotMapping slotMapping = new SlotMapping();

        /**
         * Source/sink {@link Operator} in this instance. Should only be set if the
         * surrounding {@link OperatorAlternative}
         * is a source/sink.
         */
        ...
        @Override
        public void setSink(final Operator innerSink) {
            this.sink = innerSink;
        }

        @Override
        public <T> OutputSlot<T> traceOutput(final OutputSlot<T> alternativeOutputSlot) {
            // If this alternative is not a sink, we trace the given output slot via the
            // slot mapping.
            if (!OperatorAlternative.this.isOwnerOf(alternativeOutputSlot)) {
                throw new IllegalArgumentException(
                        "Cannot enter alternative: Output slot does not belong to this alternative.");
            }
            ...
            if (resolvedSlot != null && resolvedSlot.getOwner().getParent() != OperatorAlternative.this) {
                final String msg = String.format(
                        "Cannot enter through: Owner of inner OutputSlot (%s) is not a child of this alternative (%s).",
        ...
        @Override
        public void setSource(final Operator innerSource) {
            this.source = innerSource;
        }

        @Override
        public <T> Collection<InputSlot<T>> followInput(final InputSlot<T> inputSlot) {
            ...
            for (final InputSlot<T> resolvedSlot : resolvedSlots) {
                if (resolvedSlot != null && resolvedSlot.getOwner().getParent() != OperatorAlternative.this) {
                    final String msg = String.format(
                            "Cannot enter through: Owner of inner OutputSlot (%s) is not a child of this alternative (%s).",
                            Operators.collectParents(resolvedSlot.getOwner(), true),
        ...
        @Override
        public <T> InputSlot<T> traceInput(final InputSlot<T> inputSlot) {
        ...
        @Override
        public <T> Collection<OutputSlot<T>> followOutput(final OutputSlot<T> outputSlot) {
    ...
    public static OperatorAlternative wrap(final Operator startOperator, final Operator endOperator) {
     ...
     * <ol>
     * <li>Creates a new instance that mocks the interface (slots) of the given
     * operator,</li>
     ...
     */
    public static OperatorAlternative wrap(final Operator operator) {
        // Test whether the operator is not already an alternative.
        final OperatorContainer container = operator.getContainer();
        if (container != null && container.toOperator().isAlternative()
                && container.getContainedOperators().size() == 1) {
            return (OperatorAlternative) container.toOperator();
        }

        final OperatorAlternative operatorAlternative = operator.isLoopHead()
        ...
        final Alternative alternative = operatorAlternative.addAlternative(operator);
        if (container != null) {
    ...
    /**
     * All alternatives for this operator. Note that we deliberately do not use a
     * {@link SlotMapping} at this point
     * because this can be achieved with a {@link Subplan}.
     */
    private final List<Alternative> alternatives = new LinkedList<>();

    /**
     * Creates a new instance with the same number of inputs and outputs and the
     * same parent as the given operator.
     */
    protected OperatorAlternative(final Operator operator) {
     ...
     *
     * @param alternativeOperator either an {@link ElementaryOperator} or a
     *                            {@link Subplan}; in the latter case, the
     *                            {@link Subplan} will be "unpacked" into the new
     *                            {@link Alternative}
     */
    public Alternative addAlternative(final Operator alternativeOperator) {
        final Alternative alternative = this.createAlternative();
    ...
    @Override
    public <Payload, Return> Return accept(final TopDownPlanVisitor<Payload, Return> visitor, final OutputSlot<?> outputSlot,
            final Payload payload) {
        return visitor.visit(this, outputSlot, payload);
    }

    @Override
    public void noteReplaced(final Operator oldOperator, final Operator newOperator) {
    }

    @Override
    public void propagateOutputCardinality(final int outputIndex,
            final OptimizationContext.OperatorContext operatorContext,
            final OptimizationContext targetContext) {
        super.propagateOutputCardinality(outputIndex, operatorContext, targetContext);
        this.getAlternatives()
                .forEach(alternative -> alternative.propagateOutputCardinality(outputIndex, operatorContext));
    }

    @Override
    public void propagateInputCardinality(final int inputIndex, final OptimizationContext.OperatorContext operatorContext) {
        super.propagateInputCardinality(inputIndex, operatorContext);
        this.getAlternatives()
                .forEach(alternative -> alternative.propagateInputCardinality(inputIndex, operatorContext));
    }

    @Override
    public <T> LinkedHashSet<OutputSlot<T>> collectMappedOutputSlots(final OutputSlot<T> output) {
    ...
    @Override
    public <T> LinkedHashSet<InputSlot<T>> collectMappedInputSlots(final InputSlot<T> input) {
        return Stream.concat(
                Stream.of(input),
