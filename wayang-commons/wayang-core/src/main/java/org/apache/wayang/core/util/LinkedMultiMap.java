...
/**
 * Linked implementation of {@link MultiMap}, perserves order under the usage of
 * {@link LinkedHashSet} & {@link LinkedHashMap}.
 * See {@link MultiMap} for unperserved order.
 */
public class LinkedMultiMap<K, V> extends LinkedHashMap<K, LinkedHashSet<V>> {
    /**
    ...
    public boolean putSingle(K key, V value) {
        final LinkedHashSet<V> values = this.computeIfAbsent(key, k -> new LinkedHashSet<>());
    ...
    public boolean removeSingle(K key, V value) {
        final LinkedHashSet<V> values = this.get(key);
        if (values != null) {
