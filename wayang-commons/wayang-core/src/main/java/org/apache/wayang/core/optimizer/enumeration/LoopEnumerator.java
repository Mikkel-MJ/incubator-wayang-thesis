...
public class LoopEnumerator extends OneTimeExecutable {
    ...
    public LoopEnumerator(final PlanEnumerator planEnumerator, final OptimizationContext.LoopContext loopContext) {
    ...
    protected void doExecute() {
        ...
        final OptimizationContext aggregateContext = this.loopContext.getAggregateContext();
        final LoopSubplan loop = this.loopContext.getLoop();

        // Create the end result.
        this.loopEnumeration = new PlanEnumeration();
        for (final OutputSlot<?> loopOutput : loop.getAllOutputs()) {
            if (loopOutput.getOccupiedSlots().isEmpty()) {
                this.loopEnumeration.getServingOutputSlots().add(new Tuple<>(loopOutput, null));
            } else {
                for (final InputSlot<?> input : loopOutput.getOccupiedSlots()) {
                    this.loopEnumeration.getServingOutputSlots().add(new Tuple<>(loopOutput, input));
                }
            }
        }
        for (final InputSlot<?> loopInput : loop.getAllInputs()) {
            this.loopEnumeration.getRequestedInputSlots().add(loopInput);
        }

        // Enumerate the loop body (for now, only a single loop body).
        final PlanEnumerator loopBodyEnumerator = this.planEnumerator.forkFor(this.loopContext.getLoop().getLoopHead(),
                aggregateContext);
        ...
        // Wrap each PlanImplementation in a new PlanImplementation that subsumes the
        // whole loop instead of iterations.
        for (final PlanImplementation loopBodyImplementation : loopBodyEnumeration.getPlanImplementations()) {
    ...
    /**
     * Adds feedback {@link Junction}s for all {@link PlanImplementation}s in the
     * {@code curBodyEnumeration}.
     *
     * @param curBodyEnumeration       a {@link PlanEnumeration} within which the
     *                                 feedback {@link Junction}s should be added
     * @param successorBodyEnumeration implemetns the successor iteration
     * @param optimizationContext      used for the createion of
     *                                 {@code curBodyEnumeration}
     */
    private void addFeedbackConnections(final PlanEnumeration curBodyEnumeration,
            final PlanEnumeration successorBodyEnumeration,
        ...
        final LoopHeadOperator loopHead = this.loopContext.getLoop().getLoopHead();

        // Go through all loop body InputSlots.
        for (final Iterator<PlanImplementation> iterator = curBodyEnumeration.getPlanImplementations()
                .iterator(); iterator.hasNext();) {
            final PlanImplementation loopImpl = iterator.next();
            for (final InputSlot<?> loopBodyInput : loopHead.getLoopBodyInputs()) {
     ...
     * @param occupant      {@link OutputSlot} (abstract) that provides feedback
     *                      connections
     * @param loopBodyInput {@link InputSlot} (abstract) which takes the feedback
     *                      connection
     * @return whether the adding the feedback connection was successful
     */
    private boolean addFeedbackConnection(final PlanImplementation loopImpl,
            final OutputSlot<?> occupant,
            final InputSlot<?> loopBodyInput,
            final OptimizationContext optimizationContext) {
        final List<InputSlot<?>> execLoopBodyInputs = new ArrayList<>(
                loopImpl.findExecutionOperatorInputs(loopBodyInput));
        final Collection<OutputSlot<?>> execOutputs = loopImpl.findExecutionOperatorOutput(occupant);
        for (final OutputSlot<?> execOutput : execOutputs) {
