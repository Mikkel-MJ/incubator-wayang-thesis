...
WHERE
    ...
    AND (
        chn.name LIKE '%man%' OR
        chn.name LIKE '%Man%'
    )
    AND it2.info = 'rating'
    AND k.keyword IN (
        'superhero', 'marvel-comics', 'based-on-comic',
    ...
    AND chn.surname_pcode = ak.surname_pcode
    AND chn.name_pcode_nf = ak.name_pcode_nf
    AND ak.name_pcode_cf = n.name_pcode_cf
    AND chn.name_pcode_nf = n.name_pcode_nf;
