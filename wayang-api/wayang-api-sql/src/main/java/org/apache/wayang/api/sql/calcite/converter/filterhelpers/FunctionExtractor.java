...
public class FunctionExtractor extends RexVisitorImpl<String> {
    ...
    public String visitInputRef(final RexInputRef inputRef) {
        ...
        return specifiedColumnNames[listIndex];
    }
    ...
    public String visitCall(final RexCall call) {
        ...
        if (subResults.size() == 1) {
            ...
            if (call.getOperator().getName().contains("IS"))
                return subResults.get(0) + " " + call.getOperator().getName();
            // if the rexCall (NOT) has just one child like in the case of LIKE with a
            // negation
            // i.e. NOT LIKE
